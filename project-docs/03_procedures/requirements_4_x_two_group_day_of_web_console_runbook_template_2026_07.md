...
本手順書の作業時間は、Webコンソール操作、設定値確認、スクリーンショット取得、作業台帳への記録を含む目安である。  
設定値、対象アカウント、対象リージョン、通知先SNS Topic、通知確認者、切り戻し判断者が作業前に確定していることを前提とする。

| 時間 | 主な作業内容 | 根拠 |
| :--- | :--- | :--- |
| 10分 | 1画面または1設定の確認 | 対象画面を開き、設定値または状態を確認し、必要に応じて証跡を保存する範囲である |
| 15分 | 前提確認、Trail設定確認、対応済み設定の確認 | 複数項目を作業台帳または設計資料と突合するが、設定変更を伴わない範囲である |
| 20分 | 変更パラメータ確認、EventBridge重複確認、Metric Filter作成、CloudWatch Alarm作成 | 複数画面を操作し、入力値確認、保存、作成後確認を行う範囲である |
| 25分 | 要件1件あたりの標準設定作業 | 対応区分確認、Metric Filter作成、Alarm作成、作成後確認への導線を含む標準目安である |
| 30分 | 変更前後エビデンス取得、通知テスト、切り戻し作業 | 複数画面のスクリーンショット、通知到達確認、作業前後比較を含む範囲である |

レビュー時の説明では、以下を時間見積もりの前提として扱う。

- 既存SNS Topicを利用し、新規TopicやSubscriptionは原則作成しない
- Metric Filter / Alarmの命名規則とFilter Patternは作業前に確定している
- 作業者は対象AWSアカウント、対象リージョン、対象Log Groupを把握している
- 通知テストの実施可否と通知確認者が作業前に確定している
- 実イベントを発生させるテストは、承認済みの場合のみ実施する

作業時間が増える主な要因は以下である。

- 既存EventBridge / A-gate設定との重複確認に追加調査が必要になる
- SNS Topic、Subscription、Teams連携、監視基盤連携の通知経路確認に時間を要する
- 実イベントを発生させる通知テストで、影響確認または切り戻し判断が必要になる
- 作業対象アカウント、リージョン、Log Group、Trailが複数存在する
- 設定値が作業当日に未確定である

そのため、Excel上の「作業にかかる時間」は標準目安であり、レビューまたは作業承認時には、確認待ち・通知確認・証跡整理を含めて余裕時間を別途確保する。
