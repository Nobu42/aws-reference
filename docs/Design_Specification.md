...
本構成ではコストを抑えた検証構成として、RDSはSingle-AZで作成する。
本番相当の可用性要件を満たす場合は、Multi-AZ構成への変更を検討する。
...
本リファレンスでは構成を簡潔にするためAWS管理ポリシーを使用している。
実運用では、対象バケットと必要な操作に限定した最小権限ポリシーを適用する。
...
`bastion` と `www` は構築時に作成し、削除時に削除する。
...
本構成ではRedis Cluster構成の参照例として、クラスターモード有効、2シャード、各シャード2レプリカで作成する。
...
本リファレンスでは、AWS CLIとシェルスクリプトにより必要なAWSリソースを作成し、検証完了後に削除する運用を前提とする。

課金対象リソースを明確に管理するため、日常的に作成・削除するリソースと、継続利用するリソースを分けて扱う。
...
検証完了後は `cleanup_all.sh` を実行し、課金対象となる一時リソースを削除する。

削除後は `check_cleanup.sh` を実行し、以下のリソースが残っていないことを確認する。
