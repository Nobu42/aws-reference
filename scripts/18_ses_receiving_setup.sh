...
# 既存MXや既存Active Rule Setが別設定だった場合の上書き許可。
# メール受信経路を切り替える設定なので、実運用では変更前確認が必須。
...
# Public Hosted Zoneの取り違えは、メール配送先の誤変更につながるため停止する。
...
echo "=== Default Encryption on Mail Bucket ==="

# 受信メール本文をS3へ保存するため、デフォルト暗号化を有効化する。
# 学習用ではSSE-S3を使い、KMSキー管理までは行わない。
...
ACTIVE_RULE_SET_NAME=$(aws ses describe-active-receipt-rule-set \
  --profile "$PROFILE" \
  --region "$REGION" \
  --query 'Metadata.Name' \
  --output text 2>/dev/null || true)

if [ "$ACTIVE_RULE_SET_NAME" != "None" ] && [ -n "$ACTIVE_RULE_SET_NAME" ] && [ "$ACTIVE_RULE_SET_NAME" != "$RULE_SET_NAME" ]; then
  if [ "$ALLOW_REPLACE_ACTIVE_RULE_SET" != "true" ]; then
    echo "Error: another active Receipt Rule Set already exists: $ACTIVE_RULE_SET_NAME"
    echo "Set ALLOW_REPLACE_ACTIVE_RULE_SET=true if you intentionally want to activate $RULE_SET_NAME."
    exit 1
  fi
fi
...
EXISTING_MX_VALUES=$(aws route53 list-resource-record-sets \
  --profile "$PROFILE" \
  --hosted-zone-id "$HOSTED_ZONE_ID" \
  --query "ResourceRecordSets[?Name==\`${DOMAIN_NAME_DOT}\` && Type==\`MX\`].ResourceRecords[].Value" \
  --output text)

if [ -n "$EXISTING_MX_VALUES" ] && [ "$EXISTING_MX_VALUES" != "$MX_RECORD_VALUE" ]; then
  if [ "$ALLOW_REPLACE_MX" != "true" ]; then
    echo "Error: existing MX record is different from SES receiving endpoint."
    echo "Existing MX: $EXISTING_MX_VALUES"
    echo "Desired  MX: $MX_RECORD_VALUE"
    echo "Set ALLOW_REPLACE_MX=true if you intentionally want to replace the domain mail route."
    exit 1
  fi
fi
