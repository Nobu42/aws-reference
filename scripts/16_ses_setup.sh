...
# DNSやSES Identityは公開メール認証に関わるため、重複時は誤更新を避けて停止する。
...
DKIM_TOKEN_COUNT=$(echo "$DKIM_TOKENS" | wc -w | tr -d ' ')

if [ "$DKIM_TOKEN_COUNT" -ne 3 ]; then
  echo "Error: expected 3 DKIM tokens, but got $DKIM_TOKEN_COUNT."
  echo "Please check SES DKIM attributes before updating DNS records."
  exit 1
fi
...
#
# 注意:
#   TXTレコードは同じName/Typeで複数の値を持てる。
#   このスクリプトは学習用ドメインのSPF/DMARCを管理対象としてUPSERTする。
#   実運用で既存TXT値がある場合は、上書きで消してよい値か事前に確認する。
DKIM_CHANGES=""
